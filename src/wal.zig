...
const n: usize = 32768;
const WalLevel1 = MemoryCoreWal(n);
const WalLevel2 = MemoryCoreWal(n * 2);
const WalLevel3 = MemoryCoreWal(n * 4);
const WalLevel4 = MemoryCoreWal(n * 8);
const WalLevel5 = MemoryCoreWal(n * 16);
const WalLevel6 = MemoryCoreWal(n * 32);

const WalLevel = union {
    level_1: *WalLevel1,
    level_2: *WalLevel2,
    level_3: *WalLevel3,
    level_4: *WalLevel4,
...
pub fn MemoryCoreWal(comptime size: usize) type {
    return struct {
        pub fn init(alloc: std.mem.Allocator) !*CoreWal {
            return try CoreWal.init(size, alloc);
        }
    };
}

const WriterSeeker = struct {
    usingnamespace std.io.Writer;
    usingnamespace std.io.SeekableStream;
};

pub const CoreWal = struct {
    ...
    pub fn init(size: usize, alloc: std.mem.Allocator) !*Self {
        var wal = try alloc.create(CoreWal);
    ...
    pub fn appendOwn(self: *Self, r: *Record) !void {
        ...
        if (self.getWalSize() + record_size >= self.max_size) {
...
pub const FileBasedWal = struct {
    const log = std.log.scoped(.FileBasedWal);
    const Self = @This();
    ...
    // stored in it.
    pub fn init(size: usize, alloc: std.mem.Allocator) !*Self {
        var wal = try alloc.create(FileBasedWal);
        errdefer alloc.destroy(wal);
...
fn createWal(alloc: std.mem.Allocator) !*CoreWal {
